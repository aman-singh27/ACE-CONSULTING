...
export function CompanyTable({ filters, onRowClick }: CompanyTableProps) {
  const {
    ...
    isLoading,
    isError,
  } = useQuery<{ items: CompanyItem[]; total: number }>({
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell
                  colSpan={8}
                  className="text-center py-8 text-text-error"
                >
              ...
              companies.map((company) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell
                      className={`text-right font-medium ${company.hiring_velocity_score > 0 ? "text-green-500" : company.hiring_velocity_score < 0 ? "text-red-500" : "text-text-secondary"}`}
                    >
                      ...
                      <div className="flex gap-1 flex-wrap">
                        {company.bd_tags && company.bd_tags.length > 0 ? (
                          company.bd_tags.map((tag) => (
                            <Badge
                              key={tag}
                              variant={getBadgeVariant(tag) as any}
                            >
                              {tag}
                            </Badge>
                          ))
