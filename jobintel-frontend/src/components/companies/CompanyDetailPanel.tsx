...
export function CompanyDetailPanel({ company, onClose }: CompanyDetailProps) {
    const { data: jobResponse, isLoading, isError } = useQuery<any>({
    ...
    return (
        <div className="flex flex-col h-full bg-bg-surface border-l border-border-subtle shadow-lg">
            ...
            <div className="p-6 border-b border-border-subtle sticky top-0 bg-bg-surface z-10">
                ...
                <div className="grid grid-cols-3 gap-4 p-4 bg-bg-base rounded-lg border border-border-subtle">
                    <div className="flex flex-col gap-1">
                        <span className="text-xs text-text-secondary uppercase">7d Jobs</span>
                        <span className="text-lg font-semibold text-text-primary">{company.total_postings_7d}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                        <span className="text-xs text-text-secondary uppercase">30d Jobs</span>
                        <span className="text-lg font-semibold text-text-primary">{company.total_postings_30d}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                        <span className="text-xs text-text-secondary uppercase">Velocity</span>
                        <span className={`text-lg font-semibold ${company.hiring_velocity_score > 0 ? 'text-green-500' : company.hiring_velocity_score < 0 ? 'text-red-500' : 'text-text-primary'}`}>
            ...
            <div className="p-6 overflow-y-auto flex-grow bg-bg-base">
                <h3 className="text-[16px] font-semibold text-text-primary mb-4">Recent Jobs</h3>
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-4">
                        {jobResponse.items.map((job: any) => (
                            <div key={job.id} className="p-4 rounded-md border border-border-subtle bg-bg-base flex flex-col gap-2 hover:border-accent-primary/50 transition-colors">
                                <div className="flex justify-between items-start gap-4">
                                    <h4 className="font-medium text-text-primary leading-tight">{job.title}</h4>
