...
export function CompanyHubSpotSection({ company }: CompanyHubSpotSectionProps) {
    ...
    const getSyncStatus = () => {
        if (company.hubspot_synced_at) {
            return { status: 'synced', icon: Check, color: 'text-green-500', text: 'Synced' };
        } else if (company.hubspot_company_id) {
            return { status: 'in_progress', icon: Clock, color: 'text-yellow-500', text: 'In Progress' };
        } else {
            return { status: 'not_synced', icon: X, color: 'text-red-500', text: 'Not Synced' };
        }
    };
        ...
        <div className="mt-6 mb-6 py-6 px-4 bg-bg-base border border-border-subtle rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="flex flex-col gap-1">
                    <span className="text-xs text-text-secondary uppercase">Company ID</span>
            ...
            {company.hubspot_synced_at && (
                <div className="mb-4">
                    <span className="text-xs text-text-secondary uppercase">Last Synced</span>
